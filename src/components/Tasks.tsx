...
export default function Tasks() {
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-white tracking-tight text-shadow">待办清单</h2>
        <p className="text-white/60 text-sm text-shadow">按重要程度自动排序</p>
      </div>

      <form onSubmit={handleAdd} className="relative mb-8 group z-10">
        <div className="flex flex-col gap-2 bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 p-3 shadow-xl transition-all focus-within:ring-2 focus-within:ring-blue-500/50">
          <input 
            value={input} 
            onChange={e => setInput(e.target.value)} 
            placeholder="添加一个任务..." 
            className="w-full bg-transparent px-2 py-1 text-white outline-none placeholder-white/40 text-shadow text-lg"
            autoFocus
          />
          ...
          <div className="flex items-center justify-between border-t border-white/10 pt-2 mt-1">
            <div className="flex gap-2">
              ...
              <div className="relative">
                <button 
                  ...
                  className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${dueDate ? 'bg-blue-600/30 text-blue-200' : 'bg-white/10 text-slate-300 hover:bg-white/20'}`}
                >
                  <Calendar size={12} />
            ...
            <button 
              type="submit" 
              disabled={!input.trim()}
              className="p-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl disabled:opacity-50 disabled:bg-slate-700 transition shadow-lg"
            >
              <Plus size={20} />
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3">
        <div className="space-y-3">
          {sortedPendingTasks.map(t => {
            ...
            return (
              <div key={t.id} className="group flex items-start gap-3 p-4 rounded-2xl bg-black/20 border border-white/5 hover:bg-black/40 hover:border-white/20 transition-all duration-200 backdrop-blur-sm shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  <div className={`text-base truncate text-shadow ${t.priority === 'high' ? 'text-red-100 font-bold' : 'text-white'}`}>{t.title}</div>
                  <div className="flex flex-wrap items-center gap-3 mt-1.5">
                    <button onClick={() => cyclePriority(t.id, t.priority)} className="flex items-center gap-1 text-[10px] bg-white/10 px-2 py-0.5 rounded hover:bg-white/20 transition cursor-pointer">
                      <Flag size={10} className={t.priority === 'high' ? 'text-red-400 fill-current' : t.priority === 'low' ? 'text-green-400' : 'text-slate-400'}/>
                      <span className="text-white/70">{t.priority === 'high' ? 'High' : t.priority === 'normal' ? 'Normal' : 'Low'}</span>
                    </button>
                ...
                <button onClick={() => deleteTask(t.id)} className="p-2 text-white/40 hover:text-red-400 hover:bg-white/5 rounded-lg opacity-0 group-hover:opacity-100 transition"><Trash2 size={18} /></button>
              </div>
            );
          })}
        </div>
        {doneTasks.length > 0 && (
          <div className="pt-8 border-t border-white/10 mt-4">
            <h3 className="text-xs font-bold text-white/50 uppercase mb-4 ml-1 flex items-center gap-2 text-shadow"><Check size={14}/> 已完成 ({doneTasks.length})</h3>
            <div className="space-y-2">
              {doneTasks.map(t => (
                <div key={t.id} className="flex items-center gap-3 p-3 rounded-xl bg-black/10 border border-white/5 opacity-50 hover:opacity-100 transition">
                  ...
                  <span className="text-sm text-white/60 line-through flex-1">{t.title}</span>
                  <button onClick={() => deleteTask(t.id)} className="text-white/40 hover:text-red-400 p-1"><Trash2 size={14} /></button>
                </div>
              ))}
            </div>
          </div>
